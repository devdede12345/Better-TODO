...
function extractDueTimestamp(taskText) {
    ...
    if (fullDateMatch) {
        return toValidTimestamp(parseInt(fullDateMatch[1], 10), parseInt(fullDateMatch[2], 10), parseInt(fullDateMatch[3], 10), parseInt(fullDateMatch[4], 10), parseInt(fullDateMatch[5], 10));
    }
    const monthDayMatch = taskText.match(/(?:^|\s)@(\d{2})[\/.](\d{2})\s+(\d{2}):(\d{2})(?=\s|$)/);
    if (monthDayMatch) {
        return toValidTimestamp(fallbackYear, parseInt(monthDayMatch[1], 10), parseInt(monthDayMatch[2], 10), parseInt(monthDayMatch[3], 10), parseInt(monthDayMatch[4], 10));
    }
    const compactMatch = taskText.match(/(?:^|\s)@(\d{2})(\d{2})(\d{2})(\d{2})(?=\s|$)/);
    if (compactMatch) {
        return toValidTimestamp(fallbackYear, parseInt(compactMatch[1], 10), parseInt(compactMatch[2], 10), parseInt(compactMatch[3], 10), parseInt(compactMatch[4], 10));
    }
    return null;
}
...
function formatTaskStatusTimestamp(date = new Date()) {
...
function parseTaskStatusTimestamp(raw) {
    ...
    if (!full)
        return null;
    return toValidTimestamp(parseInt(full[1], 10), parseInt(full[2], 10), parseInt(full[3], 10), parseInt(full[4], 10), parseInt(full[5], 10));
}
...
function markReminderTaskDone(reminder) {
    if (!existsSync(reminder.filePath))
        return;
...
ipcMain.handle("explorer:readDir", (_event, rootPath) => {
    if (!rootPath)
        return null;
    return buildFileTree(rootPath);
});
ipcMain.handle("explorer:openFileByPath", (_event, filePath) => {
...
ipcMain.handle("reminder:snoozeNext", (_event, delayMs) => {
    const next = getNextReminderTask();
    if (!next)
        return false;
...
ipcMain.handle("sticker:setLocked", (_event, locked) => {
...
ipcMain.handle("sticker:toggleTask", (_event, lineIndex) => {
    if (!currentFilePath || !existsSync(currentFilePath))
        return false;
...
ipcMain.on("sticker:syncContent", (_event, content, fileName) => {
    if (stickerWindow && !stickerWindow.isDestroyed()) {
        stickerWindow.webContents.send("sticker:update", content, fileName);
    }
    if (widgetWindow && !widgetWindow.isDestroyed()) {
...
ipcMain.on("reminder:syncDraft", (_event, content) => {
    if (!currentFilePath)
        return;
    syncRemindersFromContent(content, currentFilePath);
});
// ─── Quick Entry IPC ────────────────────────────────────────────────────────
ipcMain.handle("quickentry:submit", (_event, text) => {
    if (!text.trim())
        return;
...
ipcMain.handle("system:setAutoLaunch", (_event, enabled) => {
